import { Canvas, Meta, Story } from '@storybook/addon-docs';
import { ThemedContainer } from '../../helpers';
import { ArrowIcon, CopyIcon, ServerIcon } from '../../icons';
import { usePagination } from '../../pagination';
import { TableColumnV2 } from '../BaseTable.class';
import Table from './Table';
import { Button } from '.././../buttons';
import { HomeIcon } from '../../icons';

<Meta title='Components/TableV2/Table' parameters={{ options: { showToolbar: false } }} />

# Table

Visual Representation of Data in Tabular format alongwith Associated components in Table like Header and Pagination.

#### Design - [Figma](https://www.figma.com/file/e9opoAtQHBo4vFd6u0Co98/Component-Library?type=design&node-id=1-5&mode=design&t=tXbQWENNIzmFNTVU-0) | Code - [Github](https://github.com/banyandata-cloud/roots/tree/main/components/table/table)

> This components uses combination of all Table Elements i.e Cell, Header, Coloum and Row.

## Usage

Basic import and additional information on how to use this component.

```js
import { Table } from '@banyan_cloud/roots';
```

-   The `className` of the Table can be adjusted in the implementaion code to match its flexibility with respect to its container.

## Stories

The CSF rendered inside the `Canvas` from which you can see the implementation example.

### Default

<Canvas>
	<Story name='Default'>
		{() => {
			const onIntersection = (state) => {
				if (state) {
					console.log('Make API call for next Page APIS', state);
				}
			};
			const isFloating = false;
			const v2 = false;
			const onRowClick = (datum, setActiveId) => {
				console.log(datum, setActiveId);
				setActiveId();
			};
			const headerData = [
				new TableColumnV2({
					title: 'Name',
					id: 'name',
					size: 'md',
					flexible: true,
				}),
				new TableColumnV2({
					title: 'Description',
					id: 'description',
					size: 'lg',
					flexible: true,
					multiLine: true,
				}),
				new TableColumnV2({
					title: 'Gender',
					id: 'gender',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Age',
					id: 'age',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Designation',
					id: 'designation',
				}),
			];
			const tableData = [
				{
					name: '-',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 28,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'UP',
				},
				{
					name: 'Pradeep Annadurai',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 24,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Tamil Nadu',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
			];
			const [paginationState, paginationDispatch] = usePagination({
				totalPages: 20,
				currentPage: 1,
			});
			return (
				<ThemedContainer
					style={{
						height: '100%',
					}}>
					<Table
						onIntersection={onIntersection}
						isFloating={isFloating}
						v2={v2}
						tableInfo={{
							title: 'Table Title',
							description: 'Lorem ipsum dolor sit amet consectetur.',
						}}
						searchProps={{
							icon: () => {
								return <HomeIcon />;
							},
						}}
						onRowClick={onRowClick}
						headerData={headerData}
						tableData={tableData}
						emptyPlaceholder={() => {
							return <div>Hello Empty</div>;
						}}
						dataLabel='items'
						paginationData={{
							paginationState,
							paginationDispatch,
						}}
					/>
				</ThemedContainer>
			);
		}}
	</Story>
</Canvas>

### Custom Pagination

<Canvas>
	<Story name='Custom Pagination'>
		{() => {
			const onIntersection = (state) => {
				if (state) {
					console.log('Make API call for next Page APIS');
				}
			};
			const isFloating = true;
			const v2 = false;
			const onRowClick = (datum, setActiveId) => {
				console.log(datum, setActiveId);
				setActiveId();
			};
			const headerData = [
				new TableColumnV2({
					title: 'Name',
					id: 'name',
					size: 'sm',
					// flexible: true,
					sticky: 'left',
				}),
				new TableColumnV2({
					title: 'Description',
					id: 'description',
					size: 'lg',
					flexible: true,
					multiLine: true,
				}),
				new TableColumnV2({
					title: 'Gender',
					id: 'gender',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Age',
					id: 'age',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Designation',
					id: 'designation',
				}),
				new TableColumnV2({
					title: 'A Column',
					id: 'aColumn',
					size: 'lg',
					flexible: true,
				}),
				new TableColumnV2({
					title: 'Another Column',
					id: 'anotherColumn',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'Another One',
					id: 'anotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'And Another One',
					id: 'andAnotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'State',
					id: 'state',
					size: 'sm',
					sticky: 'right',
				}),
			];
			const tableData = [
				{
					name: 'Aryan Dixit',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 27,
					designation: 'UI Engineer',
					aColumn: '{"hlConfigControlMapId":["BCHLCON06005"]}',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'UP',
				},
				{
					name: 'Pradeep Annadurai',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 24,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Tamil Nadu',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
				{
					name: 'Jaidev Singh Bhui',
					gender: 'M',
					description:
						'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
					age: 23,
					designation: 'UI Engineer',
					aColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherColumn:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					anotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					andAnotherOne:
						'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
					state: 'Delhi',
				},
			];
			const [paginationState, paginationDispatch] = usePagination({
				totalPages: 60,
				currentPage: 1,
			});
			return (
				<ThemedContainer
					style={{
						height: '100%',
					}}>
					<Table
						onIntersection={onIntersection}
						isFloating={isFloating}
						v2={v2}
						tableInfo={{
							title: 'Table Title',
							description: 'Lorem ipsum dolor sit amet consectetur.',
						}}
						onRowClick={onRowClick}
						headerData={headerData}
						tableData={tableData}
						dataLabel='items'
						customPagination={true}
						customLabel='Total Records : Anirudh'
						jumpLabel='Jump to Minute'
						customPageCallback={(page) => {
							console.log('CLICKED', page);
						}}
						hideDisabledPages={false}
						customPageList={[
							{
								enable: true,
								label: '01 min',
								pageNumber: 1,
							},
							{
								enable: true,
								label: '02 min',
								pageNumber: 2,
							},
							{
								enable: false,
								label: '03 min',
								pageNumber: 3,
							},
							{
								enable: true,
								label: '04 min',
								pageNumber: 4,
							},
							{
								enable: true,
								label: '05 min',
								pageNumber: 5,
							},
							{
								enable: true,
								label: '06 min',
								pageNumber: 6,
							},
							{
								enable: true,
								label: '07 min',
								pageNumber: 7,
							},
							{
								enable: false,
								label: '08 min',
								pageNumber: 8,
							},
							{
								enable: true,
								label: '09 min',
								pageNumber: 9,
							},
							{
								enable: true,
								label: '10 min',
								pageNumber: 10,
							},
							{
								enable: true,
								label: '11 min',
								pageNumber: 11,
							},
							{
								enable: true,
								label: '12 min',
								pageNumber: 12,
							},
							{
								enable: false,
								label: '13 min',
								pageNumber: 13,
							},
							{
								enable: false,
								label: '14 min',
								pageNumber: 14,
							},
							{
								enable: false,
								label: '15 min',
								pageNumber: 15,
							},
							{
								enable: false,
								label: '16 min',
								pageNumber: 16,
							},
							{
								enable: true,
								label: '17 min',
								pageNumber: 17,
							},
							{
								enable: false,
								label: '18 min',
								pageNumber: 18,
							},
							{
								enable: true,
								label: '19 min',
								pageNumber: 19,
							},
							{
								enable: true,
								label: '20 min',
								pageNumber: 20,
							},
							{
								enable: true,
								label: '21 min',
								pageNumber: 21,
							},
							{
								enable: true,
								label: '22 min',
								pageNumber: 22,
							},
							{
								enable: true,
								label: '23 min',
								pageNumber: 23,
							},
							{
								enable: false,
								label: '24 min',
								pageNumber: 24,
							},
							{
								enable: false,
								label: '25 min',
								pageNumber: 25,
							},
							{
								enable: false,
								label: '26 min',
								pageNumber: 26,
							},
							{
								enable: false,
								label: '27 min',
								pageNumber: 27,
							},
							{
								enable: false,
								label: '28 min',
								pageNumber: 28,
							},
							{
								enable: false,
								label: '29 min',
								pageNumber: 29,
							},
							{
								enable: false,
								label: '30 min',
								pageNumber: 30,
							},
							{
								enable: false,
								label: '31 min',
								pageNumber: 31,
							},
							{
								enable: false,
								label: '32 min',
								pageNumber: 32,
							},
							{
								enable: false,
								label: '33 min',
								pageNumber: 33,
							},
							{
								enable: false,
								label: '34 min',
								pageNumber: 34,
							},
							{
								enable: false,
								label: '35 min',
								pageNumber: 35,
							},
							{
								enable: false,
								label: '36 min',
								pageNumber: 36,
							},
							{
								enable: false,
								label: '37 min',
								pageNumber: 37,
							},
							{
								enable: false,
								label: '38 min',
								pageNumber: 38,
							},
							{
								enable: false,
								label: '39 min',
								pageNumber: 39,
							},
							{
								enable: false,
								label: '40 min',
								pageNumber: 40,
							},
							{
								enable: false,
								label: '41 min',
								pageNumber: 41,
							},
							{
								enable: false,
								label: '42 min',
								pageNumber: 42,
							},
							{
								enable: false,
								label: '43 min',
								pageNumber: 43,
							},
							{
								enable: false,
								label: '44 min',
								pageNumber: 44,
							},
							{
								enable: false,
								label: '45 min',
								pageNumber: 45,
							},
							{
								enable: false,
								label: '46 min',
								pageNumber: 46,
							},
							{
								enable: false,
								label: '47 min',
								pageNumber: 47,
							},
							{
								enable: true,
								label: '48 min',
								pageNumber: 48,
							},
							{
								enable: false,
								label: '49 min',
								pageNumber: 49,
							},
							{
								enable: false,
								label: '50 min',
								pageNumber: 50,
							},
							{
								enable: false,
								label: '51 min',
								pageNumber: 51,
							},
							{
								enable: false,
								label: '52 min',
								pageNumber: 52,
							},
							{
								enable: false,
								label: '53 min',
								pageNumber: 53,
							},
							{
								enable: false,
								label: '54 min',
								pageNumber: 54,
							},
							{
								enable: false,
								label: '55 min',
								pageNumber: 55,
							},
							{
								enable: false,
								label: '56 min',
								pageNumber: 56,
							},
							{
								enable: false,
								label: '57 min',
								pageNumber: 57,
							},
							{
								enable: false,
								label: '58 min',
								pageNumber: 58,
							},
							{
								enable: false,
								label: '59 min',
								pageNumber: 59,
							},
							{
								enable: false,
								label: '60 min',
								pageNumber: 60,
							},
						]}
						paginationData={{
							paginationState,
							paginationDispatch,
						}}
					/>
				</ThemedContainer>
			);
		}}
	</Story>
</Canvas>

### With Filters

<Canvas>
	<Story name='With Filters'>
		{() => {
			const onIntersection = (state) => {
				if (state) {
					console.log('Make API call for next Page APIS');
				}
			};
			const isFloating = true;
			const onRowClick = (datum, setActiveId) => {
				console.log(datum, setActiveId);
				setActiveId();
			};
			const headerData = [
				new TableColumnV2({
					title: 'Name',
					id: 'name',
					size: 'sm',
					columnFilter: true,
				}),
				new TableColumnV2({
					title: 'Description',
					id: 'description',
					size: 'lg',
					flexible: true,
					multiLine: true,
					columnFilter: true,
				}),
				new TableColumnV2({
					title: 'Gender',
					id: 'gender',
					size: 'sm',
					columnFilter: true,
				}),
				new TableColumnV2({
					title: 'Age',
					id: 'age',
					size: 'sm',
					columnFilter: true,
				}),
				new TableColumnV2({
					title: 'Designation',
					id: 'designation',
					columnFilter: true,
				}),
				new TableColumnV2({
					title: 'A Column',
					id: 'aColumn',
					size: 'lg',
					flexible: true,
				}),
				new TableColumnV2({
					title: 'Another Column',
					id: 'anotherColumn',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'Another One',
					id: 'anotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'And Another One',
					id: 'andAnotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'State',
					id: 'state',
					size: 'sm',
				}),
			];
			const tableData = Array(10)
				.fill([
					{
						name: 'Aryan Dixit',
						gender: 'M',
						description:
							'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
						age: 27,
						designation: 'UI Engineer',
						aColumn:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						anotherColumn:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						anotherOne:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						andAnotherOne:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						state: 'UP',
					},
				])
				.flat();
			const filtersData = {
				filterValue: {
					applied: 4,
				},
			};
			const [paginationState, paginationDispatch] = usePagination({
				totalPages: 20,
				currentPage: 1,
			});
			return (
				<ThemedContainer
					style={{
						height: '100%',
					}}>
					<Table
						filtersData={filtersData}
						onIntersection={onIntersection}
						tableInfo={{
							title: 'Table Title',
							description: 'Lorem ipsum dolor sit amet consectetur.',
						}}
						disabledFilterOptions={{
							search: false,
						}}
						rightActions={() => {
							return <Button title='Action' />;
						}}
						// filtersCount={3}
						searchProps={{
							icon: () => {
								return <HomeIcon />;
							},
							onSearch: (s) => {
								console.log(s);
							},
							placeholder: 'Search Employees',
							// disabled: true,
						}}
						tableDrawerProps={{
							tabsConfig: {
								tabs: [
									{
										id: 1,
										leftIcon: {
											Active: (props) => {
												return <ServerIcon {...props} />;
											},
											InActive: (props) => {
												return <HomeIcon {...props} />;
											},
										},
									},
									{
										id: 2,
										leftIcon: {
											Active: (props) => {
												return <ServerIcon {...props} />;
											},
											InActive: (props) => {
												return <HomeIcon {...props} />;
											},
										},
									},
								],
							},
							standalone: [
								() => {
									return (
										<div data-elem='drawer-child'>
											Hello World
											<div data-elem='drawer-child-header'>
												KKKK
												<div>djkndk</div>
											</div>
											<div data-elem='drawer-child-body'>
												Nondandlk If the elements actually are siblings in
												the DOM, and the display of the first element was
											</div>
											<div data-elem='drawer-child-body'>Noss</div>
										</div>
									);
								},
							],
							renderBody: [
								() => {
									return (
										<div data-elem='drawer-child'>
											Hello World
											<div data-elem='drawer-child-header'>
												YES dsdsd dsdsdsdsd
												<div>djkndk</div>
											</div>
											<div data-elem='drawer-child-body'>
												Nondandlk If the elements actually are siblings in
												the DOM, and the display of the first element was
												toggled by something like a class dynamically
												set/removed via JavaScript, then the rest of this
												could be handled in CSS alone, using the general
												sibling combinato If the elements actually are
												siblings in the DOM, and the display of the first
												element was toggled by something like a class
												dynamically set/removed via JavaScript, then the
												rest of this could be handled in CSS alone, using
												the general sibling combinato If the elements
												actually are siblings in the DOM, and the display of
												the first element was element was toggled by
												something like a class dynamically set/removed via
												JavaScript, then the rest of this could be handled
												in CSS alone, using the general sibling combinato If
												the elements actually are siblings in the DOM, and
												the display of the first element was toggled by
												something like a class dynamically set/removed via
												JavaScript, then the rest of this could be handled
												in CSS alone, using the general sibling
												combinatoggled by something like a class dynamically
												set/removed via JavaScript, then the rest of this
												could be handled in CSS alone, using the general
												sibling combinato If the elements actually are
												siblings in the DOM, and the display of the first
												element was toggled by something like a class
												dynamically set/removed via JavaScript, then the
												rest of this could be handled in CSS alone, using
												the general sibling combinato If the elements
												actually are siblings in the DOM, and the display of
												the first element was toggled by something like a
												class dynamically set/removed via JavaScript, then
												the rest of this could be handled in CSS alone,
												using the general sibling combinato If the elements
												actually are siblings in the DOM, and the display of
												the first element was toggled by something like a
												class dynamically set/removed via JavaScript, then
												the rest of this could be handled in CSS alone,
												using the general sibling combinato If the elements
												actually are siblings in the DOM, and the display of
												the first element was toggled by something like a
												class dynamically set/removed via JavaScript, then
												the rest of this could be handled in CSS alone,
												using the general sibling combinato If the elements
												actually are siblings in the DOM, and the display of
												the first element was toggled by something like a
												class dynamically set/removed via JavaScript, then
												the rest of this could be handled in CSS alone,
												using the general sibling combinatolkndadklndsa
											</div>
											<div data-elem='drawer-child-body'>Noss</div>
										</div>
									);
								},
								() => {
									return <div>Hello Everyone</div>;
								},
							],
						}}
						isFloating={isFloating}
						onRowClick={onRowClick}
						headerData={headerData}
						tableData={tableData}
						paginationData={{
							paginationState,
							paginationDispatch,
						}}
					/>
				</ThemedContainer>
			);
		}}
	</Story>
</Canvas>

### With Default Active

<Canvas>
	<Story name='With Default Active'>
		{() => {
			const onIntersection = (state) => {
				if (state) {
					console.log('Make API call for next Page APIS');
				}
			};
			const isFloating = true;
			const onRowClick = (datum, setActiveId) => {
				console.log(datum, setActiveId);
				setActiveId();
			};
			const headerData = [
				new TableColumnV2({
					title: 'Name',
					id: 'name',
					size: 'sm',
					// flexible: true,
					sticky: 'left',
				}),
				new TableColumnV2({
					title: 'Description',
					id: 'description',
					size: 'lg',
					flexible: true,
					multiLine: true,
				}),
				new TableColumnV2({
					title: 'Gender',
					id: 'gender',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Age',
					id: 'age',
					size: 'sm',
				}),
				new TableColumnV2({
					title: 'Designation',
					id: 'designation',
				}),
				new TableColumnV2({
					title: 'A Column',
					id: 'aColumn',
					size: 'lg',
					flexible: true,
				}),
				new TableColumnV2({
					title: 'Another Column',
					id: 'anotherColumn',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'Another One',
					id: 'anotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'And Another One',
					id: 'andAnotherOne',
					size: 'lg',
				}),
				new TableColumnV2({
					title: 'State',
					id: 'state',
					size: 'sm',
					sticky: 'right',
				}),
			];
			const tableData = Array(10)
				.fill([
					{
						name: 'Pradeep',
						gender: 'M',
						description:
							'Lorem ipsum dolor sit, amet consectetur adipisicing elit. Soluta nulla voluptatum consectetur libero, fuga amet earum ducimus quidem aliquam fugit id ipsum, cupiditate dignissimos facilis, repellendus possimus aspernatur tempore! Veniam saepe dolor reprehenderit sunt tenetur quam adipisci quis deleniti, maxime officiis dolorum, iure quia excepturi velit repellendus ad eius iste.',
						age: 27,
						designation: 'UI Engineer',
						aColumn:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						anotherColumn:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						anotherOne:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						andAnotherOne:
							'Lorem ipsum dolor sit amet consectetur adipisicing elit. Ex ab dicta voluptatum pariatur architecto! Necessitatibus ut reiciendis cumque cum adipisci nesciunt unde veritatis rem consequuntur!',
						state: 'UP',
					},
				])
				.flat();
			const defaultActiveIndex = 0;
			const [paginationState, paginationDispatch] = usePagination({
				totalPages: 20,
				currentPage: 1,
			});
			return (
				<ThemedContainer
					style={{
						height: '100%',
					}}>
					<Table
						defaultActiveIndex={defaultActiveIndex}
						onIntersection={onIntersection}
						isFloating={isFloating}
						tableInfo={{
							title: 'Table Title',
							description: 'Lorem ipsum dolor sit amet consectetur.',
						}}
						onRowClick={onRowClick}
						headerData={headerData}
						tableData={tableData}
						paginationData={{
							paginationState,
							paginationDispatch,
						}}
					/>
				</ThemedContainer>
			);
		}}
	</Story>
</Canvas>

### Loading State

<Canvas>
	<Story name='Loading State'>
		{() => {
			const onIntersection = (state) => {
				if (state) {
					console.log('Make API call for next Page APIS');
				}
			};
			const isFloating = true;
			const onRowClick = (datum, setActiveId) => {
				console.log(datum, setActiveId);
				setActiveId();
			};
			const [paginationState, paginationDispatch] = usePagination({
				totalPages: 20,
				currentPage: 1,
			});
			return (
				<ThemedContainer
					style={{
						height: '100%',
					}}>
					<Table
						onIntersection={onIntersection}
						isFloating={isFloating}
						onRowClick={onRowClick}
						tableInfo={{
							title: 'Table Title',
							description: 'Lorem ipsum dolor sit amet consectetur.',
						}}
						paginationData={{
							paginationState,
							paginationDispatch,
						}}
						tableData={[]}
						loading={true}
					/>
				</ThemedContainer>
			);
		}}
	</Story>
</Canvas>

## API

**`loading`**

loading indicates the waiting state of chart while data is loading

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>bool</td>
		<td>false</td>
	</tr>
</table>

```jsx
<Table loading={false} />
```

**`headerData`**

The key that holds details of all headers of the Table mapped to the Data with key.
It has following shape - `title`, `id`, `size`, `flexible`, `sort`, `style` and `multiline`.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Array</td>
		<td>[]</td>
	</tr>
</table>

```jsx
<Table
	headerData={[
		{
			title: 'Name',
			id: 'name',
			size: 'lg',
			flexible: true,
		},
		{
			title: 'Gender',
			id: 'gender',
			size: 'sm',
		},
		{
			title: 'Age',
			id: 'age',
			size: 'sm',
		},
		{
			title: 'Designation',
			id: 'designation',
		},
		{
			title: 'State',
			id: 'state',
			size: 'sm',
		},
	]}
/>
```

**`tableData`**

The Fundamental data which is displayed in Table and mapped againt header.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Array</td>
		<td>[]</td>
	</tr>
</table>

```jsx
<Table
	tableData={[
		{
			name: 'Aryan Dixit',
			gender: 'M',
			age: 27,
			designation: 'UI Engineer',
			state: 'UP',
		},
	]}
/>
```

**`customCells`**

Any additional UI or funcationlity that is required in either Header or Body except data Mapping.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Object</td>
		<td>
			{`{
				header: null,
				body: null,
			}`}
		</td>
	</tr>
</table>

```jsx
<Table
	customCells={{
		body: BodyCells({
			datum,
			selectedResources,
			setSelectedResources,
		}),
		header: HeaderCells({
			datum,
			selectedResources,
			setSelectedResources,
			resourcesList,
		}),
	}}
/>
```

**`expandable`**
A callback function to capture Expand event.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Function</td>
		<td>() => {}</td>
	</tr>
</table>

```jsx
<Table
	expandable={() => {
		// Reset some Data
	}}
/>
```

**`onSort`**

A callback function to capture Sort event.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Function</td>
		<td>() => {}</td>
	</tr>
</table>

```jsx
<Table
	onSort={() => {
		// Reset some Data
	}}
/>
```

**`onRowClick`**

A callback function to capture Row Click event.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>Function</td>
		<td>() => {}</td>
	</tr>
</table>

```jsx
<Table
	onRowClick={() => {
		// Reset some Data
	}}
/>
```

**`className`**

Key to change style through external className.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>String</td>
		<td>''</td>
	</tr>
</table>

```jsx
<Table className='chart-area' />
```

**`rowHeight`**

Key to decide height of Row to select between `md` or `lg` mode.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>String</td>
		<td>"md"</td>
	</tr>
</table>

```jsx
<Table rowHeight='lg' />
```

**`theme`**

The color theme of the container to select between `light` or `dark` mode.

<table style={{ width: '100%' }}>
	<tr>
		<th style={{ textAlign: 'left', width: '50%' }}>TYPE</th>
		<th style={{ textAlign: 'left', width: '50%' }}>DEFAULT</th>
	</tr>
	<tr>
		<td>String</td>
		<td>"dark"</td>
	</tr>
</table>

```jsx
<Table theme='light' />
```
