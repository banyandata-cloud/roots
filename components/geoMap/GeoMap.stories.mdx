...
#### Code - [Github](https://github.com/banyandata-cloud/roots/tree/main/components/maps/GeoMap)
...
```js
import { GeoMap } from '@banyan_cloud/roots';
```
...
The default GeoMap component with tooltip in a point.
...
```jsx
<GeoMap customMarker={`url(https://someImage.com)`} />
```
...
```jsx
<GeoMap className={styles.custom} />
```
