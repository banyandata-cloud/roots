...
export const ControlledTemplate = (args) => {
	return (
		<ThemedContainer {...args}>
			<Toggle {...args} value={args.defaultSelected} />
		</ThemedContainer>
	);
};
...
<Canvas>
	<Story name='Multi Toggle'>
		{() => {
			...
			const defaultSelected = ['oracle', 'mysql', 'pgsql'];
			const [selectedToggle, setSelectedToggle] = useState(defaultSelected);
			return (
				<ThemedContainer>
					<Toggle
						options={options}
