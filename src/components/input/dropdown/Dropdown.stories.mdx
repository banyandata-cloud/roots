...
export const Template2 = (args) => (
	<ThemedContainer theme='light'>
		<Dropdownv2 {...args}>
			<DropdownItemv2 title='Option A' value={1} v2 />
			<DropdownItemv2 title='Option B' value={2} v2 />
			<DropdownItemv2 title='Option C' value={3} v2 />
		</Dropdownv2>
	</ThemedContainer>
);
...
### Default - Uncontrolled V2

The default dropdown component.

<Canvas>
	<Story
		name='Default Uncontrolled V2'
		args={{
			label: 'Default Dropdown',
			placeholder: 'Select an option',
			v2: true,
...
### Multi Select Dropdown V2

The dropdown component with multiple selections.

<Canvas>
	<Story name='Multi Select V2'>
		{() => {
			return (
				<ThemedContainer theme='light'>
					<div>
...
The dropdown component with search.

<Canvas>
	<Story name='Search V2'>
		{() => {
				...
				<ThemedContainer theme='light'>
					<div>
						<Dropdownv2
							...
							v2={true}
							label='Search Checkbox Dropdown'
							placeholder='Select an option'
							onChange={(event, newValue) => {
								setValue(newValue);
							}}>
...
<Canvas>
	<Story name='Multi Select Controlled'>
		{() => {
			const [value, setValue] = useState('');
			const [options, setOptions] = useState([...Array(50).keys()]);
			const [searchValue, setSearchValue] = useState('');
			const filteredOptions = options.filter((option) => {
				return option.toString().includes(searchValue);
			});
			return (
				<div>
					<Dropdownv2
