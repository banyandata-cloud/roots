...
const Header = ({ title }: DialogHeaderProps) => {
  return (
...
const Footer = ({
  ...
  return (
    <div className={styles.footer}>
      ...
      {onAction && (
        <Button onClick={() => onAction({ dismiss: () => setOpen(false) })} className={styles.save} title={action} color={variant} />
      )}
    </div>
  );
};
...
const DialogBox = forwardRef<DialogBoxHandle, DialogProps>((props, ref) => {
  ...
  const footerProps = ({ setNoDismissEnabled }: { setNoDismissEnabled: (enabled: boolean) => void }) => {
    return {
      action: actionText,
      cancel: cancelText,
      hideCancel,
