...
const sumArrayOfObjects = (objects) => {
...
const getSpacedDisplayName = (string) => {
	return string?.replace(/-/g, ' ').replace(/\b\w/g, (s) => {
		return s.toUpperCase();
	});
};

const doubleDigitted = (number) => {
	return number?.toString().padStart(2, '0');
};

const getJSDateFromEpoch = (epoch) => {
...
const getDatesInAMonth = ({ month, year }) => {
...
export { BASE_URLS, DAYS, FULL_MONTHS, MONTHS, classes, cloneDeep, doubleDigitted, get, getCurrentSearchParams, getDateFromEpoch, getDatesInAMonth, getInitialsOfName, getJSDateFromEpoch, getSpacedDisplayName, getTimeFromEpoch, getToday, inputHelper, stringToPath, sumArrayOfObjects, uniqueArray, uniqueArrayOfObjects, useOutsideClickListener, useResize, useRowFilter };
